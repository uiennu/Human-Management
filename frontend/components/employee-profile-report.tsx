          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
            ...
            <div>
              ...
              <div className="mt-2 relative">
                <Input placeholder="Search by name/ID..." value={employeeSearch}
                  ...
                  return (
                    <Badge key={status} variant="outline" onClick={() => toggleStatus(status)}
                      className={`cursor-pointer text-xs px-3 py-1 h-7 rounded-full border transition-all flex items-center gap-2 whitespace-nowrap ${
                        isSelected ? (status === "Active" ? "bg-green-100 text-green-700 border-green-200" :
                                      status === "On Leave" ? "bg-yellow-100 text-yellow-700 border-yellow-200" :
                                      "bg-red-100 text-red-700 border-red-200") : "bg-background text-muted-foreground"
                      }`}>
                      <div className={`w-2 h-2 rounded-full shrink-0 ${isSelected ? (status === "Active" ? "bg-green-600" : status === "On Leave" ? "bg-yellow-500" : "bg-red-600") : "bg-muted-foreground/30"}`} />
                      {status}
                    </Badge>
                  )
                })}
              ...
              <>
                      ...
                      {filteredEmployees.map((emp) => (
                        <tr key={emp.employeeId} className="border-b border-border hover:bg-muted/50">
                          ...
                          <td className="py-4 px-4">
                            <Badge variant="outline" className={`text-xs px-3 py-1 rounded-full border-0 flex w-fit items-center gap-2 ${
                                emp.status === "Active" ? "bg-green-100 text-green-700" :
                                emp.status === "On Leave" ? "bg-yellow-100 text-yellow-700" : "bg-red-100 text-red-700"
                              }`}>
                              <div className={`w-2 h-2 rounded-full ${
                                  emp.status === "Active" ? "bg-green-600" :
                                  emp.status === "On Leave" ? "bg-yellow-500" : "bg-red-600"
                                }`} />
                              {emp.status}
                            </Badge>
                          </td>
                ...
                <div className="flex flex-col md:flex-row gap-3 mt-6 items-center justify-between">
                  <div className="flex gap-3">
                    <Button onClick={() => handleExport("excel")} className="bg-green-600 hover:bg-green-700 text-white text-sm">
                      <Download className="w-4 h-4 mr-2" /> Download Excel
                    </Button>
                    <Button onClick={() => handleExport("pdf")} className="bg-blue-600 hover:bg-blue-700 text-white text-sm">
